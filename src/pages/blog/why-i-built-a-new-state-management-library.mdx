...
title: (WIP) Why I built a new state management library
---
import { Card } from "../../components/Card";

# (WIP) Why I built a new state management library

> _tl;dr [preact-signal-store](https://www.npmjs.com/package/preact-signal-store)_

## Okay, I know what you're thinking...

> _Why another state management library?
...
> How could you be so arrogant as to think you could create something that's better than all of those?_
>
> \- You, probably
...
<Card padding>
  <details>
    ...
    It fulfills all the same APIs as are exposed by React but in a more lightweight fashion.
    ...
    Where React is built to be deployed with any of several rendering engines, most notably react-dom & react-native, Preact is just built for the web and that allows it to optimize strictly for that task.
...
And if you are in a situation where you need the signal to act more like a VDOM (maybe a dependency expects full VDOM updates on state changes), then if you bind the value of a signal into the VDOM then that component will subscribe to the signal just like `useState`!
...
There would be two functions `store` and `destore` (a name I later got rightfully criticized for). The `store` function would take in an object and convert all of its deeply nested atomic values into signals. So very simply this code...
...
I hope the performance & developer experience gains justify its existance even if it is for niche set of developers who are using Preact for Preact and not just as better React.
