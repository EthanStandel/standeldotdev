...
If you check the [caniuse.com support matrix](https://caniuse.com/css-container-queries-style), it's not looking great. But it's also not nearly as bleak it it looks upon first glance. Notably, no browsers identify as fully supporting the feature, but thankfully it doesn't matter all that much. What most browsers decidedly _do not_ support is selecting based on the value of native CSS properties. But what they _do_ support is selectors based upon custom-properties (also known as CSS variables). So that is to say, if you wanted to get tricky and define your dark mode selector like the following, it wouldn't work in any browser currently.
