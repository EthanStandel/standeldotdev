...
> **Side note:** The configurations seen in the documentation exlusively uses a shorthand syntax equivalent to what's seen below, but I find the expanded syntax to be more explanatory to what a variant selector is representing. This expanded syntax is only necessary if your variant uses multiple selectors with each variant needing multiple instances of `@slot`.
...
So based on the existance of the example linked above, you might wonder why we can't use this solution right now. Unfortunately this API is very new and is still being implemented in it's entirety in browsers.
